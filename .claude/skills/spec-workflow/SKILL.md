...
`tasks.md` + sentinels + `git diff` + files on disk. Not `TaskList` output, not completion messages. Messages are laggy and lossy — **silence does not mean failure, and a receipt does not mean the work happened.** When in doubt, look at the disk.
